...
$(document).ready(function () {
	...
	var drawBadge = function () {
		...
		for (var key in shipTypes) {
			if($("#" + key.toLowerCase()).find("[type='checkbox']").length == 0){
				continue;
			}
			...
			$($("#" + key.toLowerCase()).find("[type='checkbox']").get().reverse()).each(function (i) {
				...
				if (!(useBlue && label.hasClass("blueprint") && !label.hasClass("kai"))) {
					...
					if (Math.floor(current / maxPerLine % 2) > 0) {
						x = rowbox + (maxPerLine - current % maxPerLine) * hexRectangleWidth - hexRadius;
					}
			...
			ctx.textAlign = "left";
			ctx.font = "14px " + numberfont;
			drawText(selected - blue + "/" + num, rowbox + maxPerLine * hexRectangleWidth + 8, line + newLength - 9);
			ctx.restore();
			line = line + linebarwidth/2;
			evenRow = !evenRow;
		}
		/*var maxCV = $("#cv").find("[type='checkbox']:checked").length;
		var startPositionSS = (numCVL + 3) * hexRectangleWidth;
